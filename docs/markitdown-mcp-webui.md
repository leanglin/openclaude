...
默认配置采用 MarkItDown MCP README 的官方启动方式：
...
这个 Registry 页面本身也不展示 `uvx markitdown-mcp`。真正的 MarkItDown MCP server 启动命令来自 MarkItDown 仓库里的 `packages/markitdown-mcp` README。

## 1. 安装 MarkItDown MCP 命令

有两种可用方式。主路径是官方 README 写法；`uvx` 是快捷运行方式。
...
安装后重新打开 PowerShell，再检查命令是否可用：

```powershell
markitdown-mcp --help
Get-Command markitdown-mcp
```

如果 Web UI 是从安装版应用里启动的，安装后建议完整退出并重新打开应用，让应用进程重新读取 PATH。

### 方式 B：uvx 快捷路径

如果不想先全局 `pip install markitdown-mcp`，也可以用 `uvx` 临时运行 PyPI 上的 `markitdown-mcp` 工具包。
...
1. 打开一级菜单 `工具`。
2. 切换到 `MCP Servers`。
3. 如果页面有 `MarkItDown 示例` 按钮，可以直接点击它自动填表；如果示例仍填 `uvx markitdown-mcp`，也可以手动改成下面的官方配置。
4. 如果手动填写，推荐使用官方 README 路径：
...
- 刷新当前 Web CLI 会话。
- 或新建一个聊天会话。
- 如果刚安装过 `markitdown-mcp` 或 `uv`，完整退出并重新打开安装版应用。
...
第一次运行 `uvx markitdown-mcp` 时可能需要联网下载包，等待时间会比后续运行更长。使用 `pip install markitdown-mcp` 的官方路径时，包已经提前安装。
...
```powershell
openclaude mcp add -s user markitdown -- markitdown-mcp
```
...
### 为什么 Registry 页面没有 `uvx markitdown-mcp`？

因为 `https://github.com/mcp/microsoft/markitdown` 是 MCP Registry 条目页，不是 MarkItDown MCP 包的完整安装说明。

官方 MCP 命令来自 MarkItDown 仓库的 `packages/markitdown-mcp` README。`uvx markitdown-mcp` 是使用 uv 直接运行 PyPI 工具包的快捷方式，不是 Registry 页面直接展示的命令。

### Args 到底填什么？

`Args` 就是 `Command` 后面的参数。
...
### 什么时候用 `markitdown-mcp`，什么时候用 `uvx markitdown-mcp`？

想严格按 MarkItDown 官方 README 操作，就先运行 `pip install markitdown-mcp`，然后用：
...
不想预先全局安装 Python 包，且已经有 `uvx`，可以用：
...
正确做法是在 `工具 -> MCP Servers` 中添加 `markitdown-mcp`，或者使用 `uvx markitdown-mcp` 快捷方式。

### 添加成功但模型还是不会用？

通常是当前会话还没有刷新。添加 MCP server 后需要刷新 Web CLI 会话或新建聊天。

如果刚安装过 `markitdown-mcp` 或 `uv`，还需要完整重启安装版应用，确保应用进程能找到新的命令。
...
- GitHub MCP Registry 条目：`https://github.com/mcp/microsoft/markitdown`
- MarkItDown MCP README：`https://github.com/microsoft/markitdown/tree/main/packages/markitdown-mcp`
- uv tools 文档：`https://docs.astral.sh/uv/guides/tools/`
