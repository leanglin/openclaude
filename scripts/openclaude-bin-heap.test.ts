...
const BIN_PATH = join(import.meta.dir, '..', 'bin', 'opencat')

describe('opencat launcher heap guard', () => {
  ...
  test('keeps user and troubleshooting escape hatches', () => {
    ...
    expect(source).toContain('OPENCAT_DISABLE_HEAP_RELAUNCH')
    expect(source).toContain('OPENCAT_NODE_MAX_OLD_SPACE_SIZE_MB')
