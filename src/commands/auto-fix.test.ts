...
describe('/auto-fix command prompt', () => {
  test('points project and local settings at canonical .opencat paths', async () => {
    ...
    expect(text).toContain('.opencat/settings.json')
    expect(text).toContain('.opencat/settings.local.json')
