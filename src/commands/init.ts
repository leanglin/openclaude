...
- Existing .opencat/skills/ and .claude/rules/ directories
...
If \`.opencat/skills/\` already exists with skills, review them first. Do not overwrite existing skills — only propose new ones that complement what is already there.

Create each skill at \`.opencat/skills/<skill-name>/SKILL.md\`:
...
- **Proposal-sourced hooks** (if user chose "Skills + hooks" or "Hooks only"): Consume \`hook\` entries from the Phase 3 preference queue. If Phase 2 found a formatter and the queue has no formatting hook, offer format-on-edit as a fallback. If the user chose "Neither" or "Skills only" in Phase 1, skip this bullet entirely.
  ...
  1. Target file: default based on the Phase 1 instruction-file choice — project → \`.opencat/settings.json\` (team-shared, committed); personal → \`.opencat/settings.local.json\`. Only ask if the user chose "both" in Phase 1 or the preference is ambiguous. Ask once for all hooks, not per-hook.
