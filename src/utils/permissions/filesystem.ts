...
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(
        join(getOriginalCwd(), PRODUCT_PROJECT_CONFIG_DIR_NAME, 'skills'),
      ),
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.opencat/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    ...
    normalizedPath.endsWith(`${sep}.opencat${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.opencat${sep}settings.local.json`) ||
    normalizedPath.endsWith(`${sep}.claude${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.claude${sep}settings.local.json`)
  ) {
    // Include legacy settings paths even for other projects.
    return true
  }
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  // Check if file is within .opencat/commands or .opencat/agents directories
 ...
 * - OpenCat config files (.opencat/settings.json, .opencat/commands/, .opencat/agents/)
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .opencat/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.opencat/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // .opencat/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .opencat/ only since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(
      join(getOriginalCwd(), PRODUCT_PROJECT_CONFIG_DIR_NAME, 'launch.json'),
    )
  ) {
    return {
      behavior: 'allow',
