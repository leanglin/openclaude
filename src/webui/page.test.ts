...
describe('webui page', () => {
  ...
  test('renders Settings auth and manual usage upload without visible token login', () => {
    ...
    expect(html).toContain('--success: #25845f;')
    expect(html).toContain('.settingsDot.ok')
  ...
  test('renders attachment chips and structured tool activity affordances', () => {
    ...
    expect(html).toContain('MAX_COMPOSER_ATTACHMENT_BYTES')
    expect(html).toContain('function arrayBufferToBase64(buffer)')
    expect(html).toContain('contentBase64')
    expect(html).toContain("updateAttachmentStatus(id, 'ready'")
    expect(html).toContain("sendWs({ type: 'send_message', text, attachments })")
    expect(html).toContain('state.attachments = [];')
    expect(html).not.toContain("updateAttachmentStatus(id, 'parsed')")
    expect(html).not.toContain('data-status="parsed"')
    expect(html).not.toContain('data-status="parsing"')
    expect(html).not.toContain('当前仅发送文本')
  ...
  test('renders Tools plugin marketplace controls instead of a placeholder', () => {
    ...
    expect(html).toContain('function renderMcpToolsPanel()')
    expect(html).toContain('function renderMcpToolsWorkspace()')
    expect(html).toContain('/api/plugins/marketplaces')
    expect(html).toContain('/api/plugins/install')
    expect(html).toContain('/api/plugins/uninstall')
    expect(html).toContain('/api/mcp/servers')
    expect(html).toContain('id="toolsSearch"')
    expect(html).toContain('id="toolsAddSource"')
    expect(html).toContain('id="toolsMcpName"')
    expect(html).toContain('id="toolsMcpCommand"')
    expect(html).toContain('id="toolsMcpTransport"')
    expect(html).toContain('data-tools-install')
    expect(html).toContain('data-tools-view="mcp"')
    expect(html).toContain('data-tools-mcp-add')
    expect(html).toContain('data-tools-mcp-remove')
    expect(html).toContain('MCP Servers')
    expect(html).toContain('https://glama.ai/mcp/servers')
    expect(html).toContain('github.com')
    expect(html).toContain("menu === 'tools' && state.tools.plugins.length === 0")
    expect(html).toContain("menu === 'tools' && state.tools.mcpServers.length === 0")
    expect(html).not.toContain('markitdown-mcp')
    expect(html).not.toContain('Tools panel will appear here.')
  })

  test('renders safe user-scope plugin uninstall controls and refresh behavior', () => {
    const html = renderWebUiPage()
    const uninstallStart = html.indexOf('async function uninstallToolPlugin')
    const uninstallEnd = html.indexOf('function refreshPluginSession', uninstallStart)
    const uninstallBody = html.slice(uninstallStart, uninstallEnd)
    const cancelStart = html.indexOf("workspaceView.querySelector('[data-tools-uninstall-cancel]')")
    const cancelEnd = html.indexOf("workspaceView.querySelector('[data-tools-refresh-session]')", cancelStart)
    const cancelHandler = html.slice(cancelStart, cancelEnd)

    expect(html).toContain('userInstalled')
    expect(html).toContain('projectEnabled')
    expect(html).toContain('installedScopes')
    expect(html).toContain("const scopeOrder = ['user', 'project', 'local', 'managed']")
    expect(html).toContain("toolInstalledScopes(selected).join('、') || '未安装'")
    expect(html).toContain('opencat plugin uninstall ')
    expect(html).toContain('Web UI 不能卸载这些 scope')
    expect(html).toContain('managed scope 由管理员管理')
    expect(html).toContain('data-tools-uninstall-confirmation')
    expect(html).toContain('data-tools-uninstall-keep')
    expect(html).toContain('data-tools-uninstall-delete')
    expect(html).toContain('data-tools-uninstall-cancel')
    expect(html).toContain('卸载并保留持久化数据目录')
    expect(html).toContain('卸载并删除持久化数据目录')
    expect(html).toContain('仅当这是最后一个安装 scope 时，插件 options 和 secrets 才会被清理')
    expect(html).toContain('选择“删除”会在此时一并删除该数据目录')
    expect(html).toContain('若其他 scope 仍安装，配置和数据目录都会保留')
    expect(html).toContain('卸载 user scope 不会修改当前项目的启用配置')
    expect(html).toContain('项目依赖同步后仍可能重新物化并生效')
    expect(html).toContain('uninstallToolPlugin(event.currentTarget.dataset.toolsUninstallKeep, false)')
    expect(html).toContain('uninstallToolPlugin(event.currentTarget.dataset.toolsUninstallDelete, true)')
    expect(html).toContain('state.tools.installingId || state.tools.uninstallingId')
    expect(uninstallBody).toContain("api('/api/plugins/uninstall'")
    expect(uninstallBody).toContain('body: JSON.stringify({ pluginId, deleteDataDir })')
    expect(uninstallBody).toContain('await refreshTools();')
    expect(uninstallBody).toContain('await refreshAssets(false);')
    expect(uninstallBody).toContain('reverseDependents')
    expect(uninstallBody).toContain('remainingScopes')
    expect(uninstallBody).toContain('可能属于其他项目')
    expect(uninstallBody).toContain('持久化数据目录也不会按最后一个 scope 删除')
    expect(uninstallBody).not.toContain('result.message')
    expect(uninstallBody).not.toContain("sendWs({ type: 'refresh_session' })")
    expect(cancelHandler).not.toContain('uninstallToolPlugin(')
  })

  test('keeps plugin assets read-only and links to installed plugin management', () => {
    const html = renderWebUiPage()

    expect(html).toContain("asset.readonly && asset.source === 'plugin'")
    expect(html).toContain('不能单独删除。请卸载整个插件。')
    expect(html).toContain('data-asset-plugin-management')
    expect(html).toContain("state.tools.status = 'installed'")
    expect(html).toContain("state.tools.search = ''")
    expect(html).toContain("state.tools.marketplace = ''")
    expect(html).toContain("selectMenu('tools')")
    expect(html).toContain("asset.readonly ? '' : '<button class=\"miniButton danger\"")
  })
